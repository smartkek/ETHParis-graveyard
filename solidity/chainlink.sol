...
contract OracleChainlink is Chainlinked, Ownable {
    ...
    function addrToString(address x) private pure returns (string) {
        ...
        public
    {
        ...
        // string memory jsonpath = string(abi.encodePacked("data[\"", addrStr, "\"].address.transaction_count"));
        string memory jsonpath = "context.code";
        
        req.add("path", jsonpath);
        bytes32 id = chainlinkRequest(req, ORACLE_PAYMENT);
        queries[id] = addr;
        emit Query(query, jsonpath);
        emit QueryId(id);
    }
